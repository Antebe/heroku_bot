    ...
    public void onUpdateReceived(Update update) {
        ...
        String player = msg.getFrom().getFirstName() + " " + msg.getFrom().getLastName();
            ...
            if( txt.toLowerCase().indexOf(word.toLowerCase()) != -1) {
                ...
                sendMsg(msg, "Вітаю +2! Ви вгадали слово " + " <" + word + ">. " + player + ", ваш рахунок: " + scores[ ArrayUtils.indexOf(players, player)]);
            }
            if( txt.toLowerCase().indexOf(cursed.toLowerCase()) != -1) {
                ...
                System.out.println(scores[ ArrayUtils.indexOf(players, player)]);
                sendMsg(msg, " Халепа! Cлово " + " <" + cursed + "> " + " було прокляте! -1. " + player + ", ваш рахунок: " + scores[ ArrayUtils.indexOf(players, player)]);
            }
            if(txt.equals("/join@MinorGame_bot") ) {
            ...
            if(txt.equals("/results@MinorGame_bot") ) {
//                int i;
//                int large[] = new int[5];
//
//
...
//                }
    ...
    String setRan() {
        String[] words = {"я", "дідько", "гей", "ти", "го", "лол", "ми", "маф", "ком", "ідіот", "хто", "вау", "ор", "мда", "топ", "дякую"};//словник
